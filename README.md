...
| deployment.openshiftOAuthProxy.image | string | `quay.io/openshift/origin-oauth-proxy:latest` pinned by digest | Image for the OAuth Proxy. |
...
| virtualService.additionalLabels | object, null | `{}` | Additional labels for VirtualService. Applied to every rendered VirtualService. Can be overridden or extended per route via `routes.<name>.additionalLabels`. |
...
| virtualService.routes | object, null | `{}` | Multiple VirtualServices — use when you need more than one VS for this app (e.g. external gateway + internal mesh-only rules). Each key produces a separate VS named <app>-<key>. Each entry supports the same fields as the top-level (hosts, gateways, exportTo, http, tls, tcp, additionalLabels, annotations). Per-route additionalLabels and annotations are merged on top of the top-level ones. When routes is set, the top-level hosts/gateways/http/tls/tcp fields are ignored. |
