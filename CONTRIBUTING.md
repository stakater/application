...
| Generate documentation and values schema | `mise run generate` |
...
Snapshot files in `application/tests/__snapshot__/` are generated by `mise run unit-test`. Do not edit them directly. Instead, run `mise run unit-test -u` to regenerate them, then commit the updated files.
